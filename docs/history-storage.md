...
Every newly published enabled-history run also contains an independently versioned `failure-index.json`. New sidecars use schema `2.0.0`; readers continue to support immutable v1 sidecars. The sidecar stores every finalized test outcome using deterministic, privacy-reduced test and failure fingerprints plus either a complete allowlisted retry/status sequence or `attempts: null`. It is built and strictly validated in the staging directory, included in the same atomic directory rename, remains present when raw `execution.json` preservation is disabled, and is removed only with its owning run.

`summary.json` may contain a `failureIndex` descriptor with schema version, fixed relative path, byte size, SHA-256 checksum, and indexed test/failure counts. Its schema must match the embedded index. This additive descriptor does not change history schema `1.0.0` or the history format version. Old summaries remain valid and are never rewritten. Migrated legacy sources publish v2 with `attempts: null`; retry count alone never claims complete history. The sidecar is not a manifest artifact type and a corrupt sidecar does not invalidate an otherwise usable summary for metrics or pass-rate analysis. See [Failure Trends](failure-trends.md) and [Durable Flaky Attempt History](flaky-attempt-history.md).
