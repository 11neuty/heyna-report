...
class HeynaHtmlDashboardGenerator {
    static async generate(options = {}) {
        ...
        const html = this.document({ metadata, summary, executionData, coverage, failureGroups, insights, rootCauses });

        fs.writeFileSync(outputPath, html, 'utf8');
        return outputPath;
    }

    static document({ metadata, summary, executionData, coverage, failureGroups, insights, rootCauses }) {
...
<body>
  <main class="dashboard-shell">
    ...
    ${this.intelligentFailureSummary(insights)}
    ${this.rootCauseAnalysis(rootCauses)}
    ${this.traceIntelligence(executionData)}
    ${this.failureGroupSummary(failureGroups)}
...
const cards = [
  ...
  <div class="summary-grid">
    ...
    static intelligentFailureSummary(insights) {
        if (!insights) return '';

        const healthLabel = insights.healthStatus;
        const badgeClass = 'health-' + healthLabel.toLowerCase();

        let html = `<section class="panel" id="intelligent-summary" aria-labelledby="insights-heading">
  <div class="section-heading">
    <h2>Intelligent Failure Summary</h2>
    <span class="health-badge ${badgeClass}">${this.escape(healthLabel)}</span>
  </div>
  <div class="insights-health">
    <strong>Total Failed Tests: </strong><span class="insights-failed-count">${insights.totalFailed}</span>
  </div>`;

        if (!insights.hasFailures) {
            html += `<div class="insights-callout insights-callout-healthy">
      <strong>${this.escape(insights.recommendation.text)}</strong>
    </div>`;
        } else {
        ...
        <div class="table-wrap"><table>
          ...
          <tbody>${insights.distribution.map(d => {
                    const catClass = 'category-' + d.category.toLowerCase();
                    return `<tr><td><span class="failure-category ${catClass}">${this.escape(d.category)}</span></td><td><strong>${d.count}</strong> (${d.percentage}%)</td></tr>`;
                }).join('\n          ')}</tbody>
        </table></div>
      </div>`;
            }
            ...
            if (insights.topRecurring.length) {
                html += `<div class="insights-section">
        <h3>Top Recurring Failures</h3>
        <div class="table-wrap"><table>
          <thead><tr><th scope="col">Signature</th><th scope="col">Category</th><th scope="col">Occurrences</th></tr></thead>
          <tbody>${insights.topRecurring.map(r => {
                    const catClass = 'category-' + r.category.toLowerCase();
                    return `<tr><td><code>${this.escape(r.signature)}</code></td><td><span class="failure-category ${catClass}">${this.escape(r.category)}</span></td><td><strong>${r.occurrences}</strong></td></tr>`;
                }).join('\n          ')}</tbody>
        </table></div>
      </div>`;
            }
            ...
            if (insights.impactedSuites.length) {
                html += `<div class="insights-section">
        <h3>Most Impacted Test Suites</h3>
        <div class="table-wrap"><table>
          <thead><tr><th scope="col">Suite</th><th scope="col">Failed Tests</th></tr></thead>
          <tbody>${insights.impactedSuites.map(s => `<tr><td>${this.escape(s.suite)}</td><td><strong>${s.failedCount}</strong> (${s.percentage}%)</td></tr>`).join('\n          ')}</tbody>
        </table></div>
      </div>`;
            }

            html += `<div class="insights-callout insights-callout-recommendation">
      <strong>Investigation Recommendation:</strong>
      <p>${this.escape(insights.recommendation.text)}</p>
      ${insights.recommendation.details.length ? `<ul>${insights.recommendation.details.map(d => `<li>${this.escape(d)}</li>`).join('')}</ul>` : ''}
    </div>`;
        }
    ...
    static rootCauseAnalysis(rootCausesData) {
        if (!rootCausesData || !rootCausesData.rootCauses || !rootCausesData.rootCauses.length) {
            return `<section class="panel" id="root-cause-analysis" aria-labelledby="rca-heading">
  <div class="section-heading">
    <h2>Root Cause Analysis</h2>
  </div>
  <p class="empty-state">No root cause clusters identified. All failures appear independent.</p>
</section>`;
        }

        const { rootCauses, totalFailuresClustered, totalClusters, coverage, unclusteredCount } = rootCausesData;

        const cards = rootCauses.map(rc => {
            const confClass = 'conf-' + rc.confidenceLabel.toLowerCase();
            const confColor = rc.confidenceLabel === 'HIGH' ? '#16A05D'
                : rc.confidenceLabel === 'MEDIUM' ? '#F59E0B'
                : '#D32F2F';

            return `<article class="rca-card">
    <div class="rca-header">
      <div>
        <strong class="rca-title">${this.escape(rc.id)}  ${this.escape(rc.label)}</strong>
        <span class="rca-root-cause">${this.escape(rc.rootCause)}</span>
      </div>
      <span class="rca-confidence ${confClass}" style="--conf-color:${confColor}">${this.escape(rc.confidenceLabel)} (${rc.confidence})</span>
    </div>
    <div class="rca-body">
      <div class="rca-meta">
        <span><strong>Category:</strong> <span class="failure-category category-${rc.category.toLowerCase()}">${this.escape(rc.category)}</span></span>
        <span><strong>Occurrences:</strong> ${rc.occurrences} (${rc.percentage}%)</span>
        <span><strong>Suites:</strong> ${rc.affectedSuites.map(s => this.escape(s)).join(', ')}</span>
      </div>
      <div class="rca-evidence">
        <strong>Evidence:</strong>
        <ul>${rc.evidence.map(e => `<li>${this.escape(e)}</li>`).join('')}</ul>
      </div>
      <div class="rca-recommendation">
        <strong>Recommendation:</strong>
        <p>${this.escape(rc.recommendation)}</p>
      </div>
    </div>
  </article>`;
        }).join('\n    ');
  ...
  <div class="section-heading">
    <h2>Root Cause Analysis</h2>
    <span class="rca-summary">${totalFailuresClustered} failures · ${totalClusters} cluster${totalClusters !== 1 ? 's' : ''} · ${coverage}% coverage${unclusteredCount ? ` · ${unclusteredCount} unclustered` : ''}</span>
  </div>
  <div class="rca-list">
    ${cards}
  </div>
    ...
    static failureGroupSummary(failureGroups) {
        if (!failureGroups || !failureGroups.length) {
            return '';
        }

        const totalFailures = failureGroups.reduce((sum, g) => sum + g.occurrences, 0);

        return `<section class="panel" id="failure-groups" aria-labelledby="failure-groups-heading">

  <div class="section-heading">
    <h2>Failure Group Summary</h2>
    <p>${totalFailures} failures in ${failureGroups.length} group${failureGroups.length !== 1 ? 's' : ''}</p>
  </div>
  <div class="table-wrap"><table>
    <thead><tr>
      ...
      <th scope="col">Affected Tests</th>
    </tr></thead>
    <tbody>${failureGroups.map(g => {
      const categoryClass = 'category-' + g.category.toLowerCase();
      return `<tr>
      <td><span class="failure-category ${categoryClass}">${this.escape(g.category)}</span></td>
      <td><code>${this.escape(g.signature)}</code></td>
      <td><strong>${g.occurrences}</strong></td>
      <td>${g.testCases.map(tc => this.escape(tc)).join(', ')}</td>
    </tr>`;
    }).join('\n    ')}</tbody>
  </table></div>
</section>`;
    ...
    static traceIntelligence(executionData) {
        ...
        if (!tracedTests.length && !absentTests.length) {
            return `<section class="panel" id="trace-intelligence" aria-labelledby="trace-heading">
  <div class="section-heading">
    <h2>Trace Intelligence</h2>
  </div>
  <p class="empty-state">No trace artifacts detected.</p>
</section>`;
        }
  ...
  <p class="empty-state">No trace artifacts were collected during this execution.</p>
</section>`;
        }

        const rows = tracedTests.map(tc => {
            const tracePath = (tc.traceFile || '').replace(/\\/g, '/');
            return `<tr>
      <td>${this.escape(tc.testCase)}</td>
      <td><span class="trace-status trace-available">Available</span></td>
      <td>${this.escape(this.formatFileSize(tc.traceSize))}</td>
      <td><a href="../${this.escape(tracePath)}" download class="trace-link">${this.escape(tc.traceFile || '-')}</a></td>
    </tr>`;
        }).join('\n    ');
  ...
  <div class="section-heading">
    <h2>Trace Intelligence</h2>
    <span>${tracedTests.length} test${tracedTests.length !== 1 ? 's' : ''} with traces</span>
  </div>
  <div class="table-wrap"><table>
    <thead><tr>
